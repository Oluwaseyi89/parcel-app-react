...
import { AlertCircle, CheckCircle, Mail, Lock, Package } from 'lucide-react';

const Courier: React.FC = () => {
  const [courcred, setCourCred] = useState({ email: '', password: '' });
  const navigate = useNavigate();

  const [logcouralert, setLogCourAlert] = useState<string | undefined>(undefined);
  const [logcour, setLogCour] = useState<any>({ first_name: '' });
  ...
  const handleCourCredChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleResetPassword = (e: React.FormEvent) => {
    ...
    const apiURL = 'http://localhost:7000/parcel_backends/courier_resetter/';
    if (resetEmail === '') {
      ...
      UseFetchJSON(apiURL, 'POST', data)
        .then((res: any) => {
          ...
          else if (res.status === 'error') setLogCourAlert(res.data);
          else setLogCourAlert('An error occured!');
        })
        .catch((err: any) => console.log(err));
    }
  };

  const handleCourCredLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setLogCourAlert(undefined);
    if (email && password) {
      const apiURL = 'http://localhost:7000/parcel_backends/courier_login/';
      UseFetchJSON(apiURL, 'POST', courcred)
        .then((res: any) => {
          if (res.data?.email === email) {
            setLogCour(res.data);
            if (res.data?.first_name) {
              localStorage.setItem('logcour', JSON.stringify(res.data));
        ...
        .catch((err: any) => console.log(err));
    } else setLogCourAlert('Please fill in all required fields');
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {logcouralert && (
          <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded-r">
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-red-700">{logcouralert}</p>
              </div>
        ...
        {/* Login Form */}
        <form onSubmit={handleCourCredLogin} className="mt-8 space-y-6 bg-white p-8 rounded-xl shadow-sm border border-gray-200">
        ...
        {/* Courier Benefits Section */}
        <div className="mt-8 bg-blue-50 border border-blue-100 rounded-lg p-6">
          <h4 className="font-medium text-blue-800 mb-3">Courier Benefits</h4>
          <ul className="space-y-2">
            <li className="flex items-start">
              <CheckCircle className="h-5 w-5 text-green-500 mr-2 flex-shrink-0 mt-0.5" />
              <span className="text-sm text-blue-700">Flexible working hours</span>
            </li>
            <li className="flex items-start">
              <CheckCircle className="h-5 w-5 text-green-500 mr-2 flex-shrink-0 mt-0.5" />
              <span className="text-sm text-blue-700">Competitive delivery rates</span>
            </li>
            <li className="flex items-start">
              <CheckCircle className="h-5 w-5 text-green-500 mr-2 flex-shrink-0 mt-0.5" />
              <span className="text-sm text-blue-700">Real-time delivery tracking</span>
            </li>
            <li className="flex items-start">
              <CheckCircle className="h-5 w-5 text-green-500 mr-2 flex-shrink-0 mt-0.5" />
